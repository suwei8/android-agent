            ...
            return try {
                when {
                    ...
                    session.method == Method.GET && session.uri == "/api/tunnel/status" -> {
                        val status = runBlocking { TunnelRuntime.refreshStatus(requireContext()) }
                        jsonResponse(
                            JSONObject()
                                .put("ok", true)
                    ...
                    session.method == Method.POST && session.uri == "/api/tunnel/bind" -> {
                        val body = parseJsonBody(session)
                        val token = body?.optString("token")?.trim().orEmpty()
                        val domain = body?.optString("domain")?.trim().orEmpty()
                        if (token.isBlank()) {
                            jsonResponse(
                                JSONObject().put("ok", false).put("error", "token 不能为空。"),
                                status = Response.Status.BAD_REQUEST,
                            )
                        } else {
                            val status = runBlocking { TunnelRuntime.bind(requireContext(), token, domain) }
                            jsonResponse(
                                JSONObject()
                                    .put("ok", true)
                                    .put("tunnel", tunnelJson(status))
                    ...
                    session.method == Method.POST && session.uri == "/api/tunnel/download" -> {
                        val status = runBlocking { TunnelRuntime.downloadBinary(requireContext(), force = true) }
                        jsonResponse(
                            JSONObject()
                                .put("ok", true)
                                .put("tunnel", tunnelJson(status))
                    ...
                    session.method == Method.POST && session.uri == "/api/tunnel/start" -> {
                        val status = runBlocking { TunnelRuntime.start(requireContext()) }
                        jsonResponse(
                            JSONObject()
                                .put("ok", status.running)
                                .put("tunnel", tunnelJson(status)),
                            status = if (status.running) Response.Status.OK else Response.Status.BAD_REQUEST,
                        )
                    }

                    session.method == Method.POST && session.uri == "/api/tunnel/stop" -> {
                        val status = runBlocking { TunnelRuntime.stop(requireContext()) }
                        jsonResponse(
                            JSONObject()
                                .put("ok", true)
                                .put("tunnel", tunnelJson(status))
